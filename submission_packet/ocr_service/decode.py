# Greedy CTC decode. No lexicon or language model — whatever the net says, we print.

import torch
from config import CHARSET, BLANK

# idx 0 is blank, real chars start at idx 1
...
def decode_ctc(logits):
    # logits: [T, B, C]. For each batch, collapse repeats + drop blanks.
    best = logits.argmax(dim=-1).cpu()   # [T, B]
    out = []
